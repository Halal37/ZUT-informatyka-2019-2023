...
class Steganography:
    def water_mark(self, img, mask, alpha=0.25):
    ...
    def put_data(self, img, data, binary_mask=np.uint8(1)):
        ...
        if dataspace == unpacked_data.size:
            ...
            prepered_data = np.resize(unpacked_data, (img.shape[0], img.shape[1], np.sum(un_binary_mask))).astype(
                np.uint8)
    ...
    def pop_data(self, img, binary_mask=np.uint8(1), out_shape=None):
        un_binary_mask = np.unpackbits(binary_mask)
