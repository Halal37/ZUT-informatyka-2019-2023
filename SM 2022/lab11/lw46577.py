...
class CorrectionCodes:
    ...
    def Write(data, filePrefix='', bit_count=4):
        ...
        for i in tqdm(range(0, bits.size, bit_count), desc="Coding for {} \t bits: ".format(bit_count)):
            ...
            sample = np.append(sample,
                               np.zeros(((bit_count - (sample.shape[0] % bit_count)) % bit_count,), dtype=np.uint8))
        ...
        for file in fl:
            ...
            if pr[0] == '0':
                ...
                with open(file, 'rb') as f:
                    ...
                    is_correct = True
                    bit_count = int(pr[1])
                    for i in tqdm(range(0, m.shape[0] - 5, (bit_count + 4)),
                                  desc="Decoding file {}: \t\t".format(file)):
    ...
    def decoding_M4(data):
        ...
        if ((CorrectionCodes.parity(data[0:-4:2]) == data[-4]) and (CorrectionCodes.parity(data[1:-4:2]) == data[-3])
                and (CorrectionCodes.parity(data[0:m]) == data[-2]) and (
                        CorrectionCodes.parity(data[m:-4]) == data[-1])):
            return data[0:-4]
        return None
...
# # Example test for text file
test = open('text.txt', 'r')
test = test.readlines()
test = np.array(test)

# Example test for picture
#test = (cv2.imread('parrot.jpg'))
...
#Zapis
CorrectionCodes.Write(test, bit_count=247)
